...
RESULTS_SUBDIR = "SUPERVISED_SSD_RECOMMENDATION_FAST_SEPARATE_WINS"
PRESENTATION_SUBDIR = "presentation_plots"
...
def get_presentation_dir():
    d = os.path.join(RESULTS_DIR, PRESENTATION_SUBDIR)
    os.makedirs(d, exist_ok=True)
    return d


def add_bar_labels(ax, bars, fmt="{:.0f}", fontsize=10):
    for b in bars:
        h = b.get_height()
        ax.annotate(
            fmt.format(h),
...
def save_presentation_counts_csv(df, filename):
    path = os.path.join(get_presentation_dir(), filename)
    df.to_csv(path, index=False)
    return path


def plot_actual_vs_predicted_wins(unique_predictions, prefix):
    outdir = get_presentation_dir()
    df = unique_predictions.copy()

    counts = pd.DataFrame([
        {"category": "Actual CP-SSD wins", "count": int((df["true_winner"] == "CP").sum())},
        {"category": "Actual KB-KSSD wins", "count": int((df["true_winner"] == "KB").sum())},
        {"category": "Predicted CP-SSD wins", "count": int((df["pred_winner"] == "CP").sum())},
        {"category": "Predicted KB-KSSD wins", "count": int((df["pred_winner"] == "KB").sum())},
    ])
    csv_path = save_presentation_counts_csv(counts, f"{prefix}_presentation_actual_vs_predicted_wins.csv")

    fig, ax = plt.subplots(figsize=(9, 4.8))
    bars = ax.bar(counts["category"], counts["count"])
    add_bar_labels(ax, bars)
    ax.set_ylabel("Number of JAGT time series")
    ax.set_title("Actual SSD winners vs recommender predictions")
    ax.set_xticks(np.arange(len(counts)))
    ax.set_xticklabels(counts["category"], rotation=18, ha="right")
    prettify_axes(ax)
    plt.tight_layout()
    path = os.path.join(outdir, f"{prefix}_presentation_actual_vs_predicted_wins.png")
...
def plot_correct_vs_false_predictions(unique_predictions, prefix):
    outdir = get_presentation_dir()
    df = unique_predictions.copy()
    ...
    counts = pd.DataFrame([
        {"category": "Correct CP-SSD recommendations", "count": correct_cp},
        {"category": "False CP-SSD recommendations", "count": false_cp},
        {"category": "Correct KB-KSSD recommendations", "count": correct_kb},
        {"category": "False KB-KSSD recommendations", "count": false_kb},
    ])
    csv_path = save_presentation_counts_csv(counts, f"{prefix}_presentation_correct_vs_false_predictions.csv")
    ...
    prettify_axes(ax)
    plt.tight_layout()
    path = os.path.join(outdir, f"{prefix}_presentation_correct_vs_false_predictions.png")
    plt.savefig(path, dpi=300)
    plt.close()
...
def plot_confusion_matrix_presentable(unique_predictions, prefix):
    outdir = get_presentation_dir()
    df = unique_predictions.copy()
    ...
    matrix = pd.crosstab(
        df["true_winner"],
        df["pred_winner"],
        rownames=["Actual winner"],
        colnames=["Predicted winner"],
    ).reindex(index=["CP", "KB"], columns=["CP", "KB"], fill_value=0)

    csv_path = os.path.join(outdir, f"{prefix}_presentation_confusion_matrix.csv")
    ...
    ax.set_xticklabels(["Predicted CP-SSD", "Predicted KB-KSSD"])
    ax.set_yticklabels(["Actual CP-SSD", "Actual KB-KSSD"])

    for i in range(2):
        for j in range(2):
            ax.text(j, i, str(int(matrix.values[i, j])), ha="center", va="center", fontsize=16)

    ax.set_title("SSD recommendation confusion matrix")
    plt.setp(ax.get_xticklabels(), rotation=15, ha="right")
    fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    plt.tight_layout()
    path = os.path.join(outdir, f"{prefix}_presentation_confusion_matrix.png")
    plt.savefig(path, dpi=300)
    plt.close()
...
def plot_accuracy_against_baselines(meta, best, prefix):
    outdir = get_presentation_dir()

    majority_acc = float((meta["true_winner"] == "KB").mean())
    best_holdout_acc = float(best["accuracy"])

    counts = pd.DataFrame([
        {"category": "Random baseline", "accuracy_percent": 50.0},
        {"category": "Majority KB baseline", "accuracy_percent": 100.0 * majority_acc},
        {"category": "TSFresh recommender\n(mean holdout)", "accuracy_percent": 100.0 * best_holdout_acc},
    ])
    csv_path = save_presentation_counts_csv(counts, f"{prefix}_presentation_accuracy_against_baselines.csv")

    fig, ax = plt.subplots(figsize=(8, 4.8))
    bars = ax.bar(counts["category"], counts["accuracy_percent"])
    add_bar_labels(ax, bars, fmt="{:.1f}%")
    ax.set_ylabel("Accuracy (%)")
    ax.set_ylim(0, 100)
    ax.set_title("SSD recommendation improves over simple baselines")
    ax.set_xticks(np.arange(len(counts)))
    ax.set_xticklabels(counts["category"], rotation=0, ha="center")
    prettify_axes(ax)
    plt.tight_layout()
    path = os.path.join(outdir, f"{prefix}_presentation_accuracy_against_baselines.png")
    plt.savefig(path, dpi=300)
    plt.close()
...
def plot_metric_summary(best, prefix):
    outdir = get_presentation_dir()

    metrics = pd.DataFrame([
        {"metric": "Accuracy", "value_percent": 100.0 * float(best["accuracy"])},
        {"metric": "Balanced accuracy", "value_percent": 100.0 * float(best["balanced_accuracy"])},
        {"metric": "CP precision", "value_percent": 100.0 * float(best["cp_precision"])},
        {"metric": "CP recall", "value_percent": 100.0 * float(best["cp_recall"])},
        {"metric": "CP F1", "value_percent": 100.0 * float(best["cp_f1"])},
    ])
    csv_path = save_presentation_counts_csv(metrics, f"{prefix}_presentation_metric_summary.csv")

    fig, ax = plt.subplots(figsize=(8.5, 4.8))
    bars = ax.bar(metrics["metric"], metrics["value_percent"])
    add_bar_labels(ax, bars, fmt="{:.1f}%")
    ax.set_ylabel("Score (%)")
    ax.set_ylim(0, 100)
    ax.set_title("Best recommender metrics")
    ax.set_xticks(np.arange(len(metrics)))
    ax.set_xticklabels(metrics["metric"], rotation=18, ha="right")
    prettify_axes(ax)
    plt.tight_layout()
    path = os.path.join(outdir, f"{prefix}_presentation_metric_summary.png")
    plt.savefig(path, dpi=300)
    plt.close()
...
def plot_top_features_presentable(importances, prefix, top_n=10):
    outdir = get_presentation_dir()

    if importances is None or importances.empty:
        return None, None

    sub = importances.head(top_n).copy()
    sub["short_feature"] = sub["feature"].astype(str).str.replace("value__", "", regex=False)
    sub["short_feature"] = sub["short_feature"].str.slice(0, 70)
    sub = sub.sort_values("mean_importance", ascending=True)

    csv_path = os.path.join(outdir, f"{prefix}_presentation_top_{top_n}_features.csv")
    importances.head(top_n).to_csv(csv_path, index=False)

    fig, ax = plt.subplots(figsize=(10, 5.5))
    y = np.arange(len(sub))
    ax.barh(y, sub["mean_importance"])
    ax.set_yticks(y)
    ax.set_yticklabels(sub["short_feature"], fontsize=8)
    ax.set_xlabel("Mean feature importance")
    ax.set_title(f"Top {top_n} TSFresh features used by the recommender")
    prettify_axes(ax)
    plt.tight_layout()
    path = os.path.join(outdir, f"{prefix}_presentation_top_{top_n}_features.png")
    plt.savefig(path, dpi=300)
    plt.close()
...
def make_presentation_readme(prefix, plot_paths):
    outdir = get_presentation_dir()
    path = os.path.join(outdir, f"{prefix}_presentation_plots_README.txt")

    lines = []
    lines.append("Presentation-ready SSD recommendation plots")
    lines.append("=" * 55)
    lines.append("")
    lines.append("Recommended order for meeting slides:")
    lines.append("1. actual_vs_predicted_wins")
    lines.append("2. accuracy_against_baselines")
    lines.append("3. confusion_matrix or correct_vs_false_predictions")
    lines.append("4. metric_summary")
    lines.append("5. top_10_features")
    lines.append("")
...
def make_all_presentation_plots(unique_predictions, meta, best, importances, prefix):
    plot_paths = []

    p, _ = plot_actual_vs_predicted_wins(unique_predictions, prefix)
    plot_paths.append(p)

    p, _ = plot_correct_vs_false_predictions(unique_predictions, prefix)
    plot_paths.append(p)

    p, _ = plot_confusion_matrix_presentable(unique_predictions, prefix)
    plot_paths.append(p)

    p, _ = plot_accuracy_against_baselines(meta, best, prefix)
    plot_paths.append(p)

    p, _ = plot_metric_summary(best, prefix)
    plot_paths.append(p)

    p, _ = plot_top_features_presentable(importances, prefix, top_n=10)
    plot_paths.append(p)

    readme = make_presentation_readme(prefix, plot_paths)

    return plot_paths, readme


def main():
    print("\nSupervised TSFresh SSD recommendation experiment")
    print("=" * 70)
    ...
    # TODO remove
    print("\n=== JAGT INDEX CHECK ===")
    print("Total rows:", len(jagt_methods))
    print("Unique keys:", jagt_methods.index.nunique())

    dups = jagt_methods.index[jagt_methods.index.duplicated()]

    print("Duplicated keys:", len(dups))

    if len(dups) > 0:
        print("Examples:")
        print(sorted(set(dups))[:20])

    print("\n=== KB/CP ERROR COLUMN AUDIT ===")

    tmp = jagt_methods.copy()

    for col in ["kb_abs_err", "cp_best_abs_err"]:
        tmp[col + "_numeric"] = pd.to_numeric(tmp[col], errors="coerce")

    print("Rows:", len(tmp))
    print("kb_abs_err NaN after numeric:", tmp["kb_abs_err_numeric"].isna().sum())
    print("cp_best_abs_err NaN after numeric:", tmp["cp_best_abs_err_numeric"].isna().sum())

    bad = tmp[
        tmp["kb_abs_err_numeric"].isna()
        | tmp["cp_best_abs_err_numeric"].isna()
        ].copy()

    # TODO remove
    ties = tmp[tmp["kb_abs_err_numeric"] == tmp["cp_best_abs_err_numeric"]]
    print("Exact ties:", len(ties))
    print(ties[["key", "kb_abs_err", "cp_best_abs_err"]].to_string(index=False))

    print("\nBad rows:", len(bad))
    print(bad[["key", "kb_abs_err", "cp_best_abs_err"]].to_string(index=False))
    ...
    # TODO remove
    cached_jagt = set(result.series_keys) & set(jagt_methods.index)
    all_jagt = set(jagt_methods.index)

    print("Total JAGT:", len(all_jagt))
    print("JAGT in cache:", len(cached_jagt))
    print("Missing:", len(all_jagt - cached_jagt))

    print("\nMissing keys:")
    for k in sorted(all_jagt - cached_jagt):
        print(k)
    ...
    print(f"\nDecisive JAGT prediction dataset used for supervised evaluation: n={len(meta)}")
    print(meta["true_winner"].value_counts())

    metrics, predictions, feature_importances = run_supervised_holdout(X, meta)

    metrics.to_csv(os.path.join(RESULTS_DIR, f"{prefix}_supervised_metrics_all_splits.csv"), index=False)
    predictions.to_csv(os.path.join(RESULTS_DIR, f"{prefix}_supervised_predictions_all_test_appearances.csv"), index=False)
    feature_importances.to_csv(os.path.join(RESULTS_DIR, f"{prefix}_supervised_feature_importances_all.csv"), index=False)

    summary = (
        metrics.groupby(["model", "feature_selection", "top_n"], as_index=False)
        .agg(
            accuracy=("accuracy", "mean"),
            balanced_accuracy=("balanced_accuracy", "mean"),
            macro_f1=("macro_f1", "mean"),
    ...
    summary.to_csv(os.path.join(RESULTS_DIR, f"{prefix}_supervised_model_summary.csv"), index=False)
    plot_supervised_model_ranking(summary, prefix)

    best = summary.iloc[0]
    best_model = str(best["model"])
    best_fs = str(best["feature_selection"])
    best_top_n = int(best["top_n"])

    unique_predictions = aggregate_unique_series_predictions_for_model(
        predictions,
        model=best_model,
    ...
    unique_predictions.to_csv(
        os.path.join(RESULTS_DIR, f"{prefix}_best_model_unique_series_predictions.csv"),
        index=False,
    )

    model_label = f"{best_model}, {best_fs}, top {best_top_n}"
    counts, plot_path, counts_path = plot_best_supervised_summary(
        unique_predictions,
        prefix,
        model_label,
    )

    importances = summarize_feature_importances(
        feature_importances,
        best_model,
        best_fs,
        best_top_n,
    ...
    presentation_plot_paths, presentation_readme = make_all_presentation_plots(
    ...
    print(plot_path)
    print(counts_path)
    print(os.path.join(RESULTS_DIR, f"{prefix}_supervised_model_summary.csv"))
    print(os.path.join(RESULTS_DIR, f"{prefix}_supervised_model_ranking_accuracy.png"))
    print(os.path.join(RESULTS_DIR, f"{prefix}_best_model_feature_importances.csv"))
    print(os.path.join(RESULTS_DIR, f"{prefix}_best_model_feature_importances.png"))
    print(os.path.join(RESULTS_DIR, f"{prefix}_FULL_JAGT_separate_wins_audit.csv"))
    print(os.path.join(RESULTS_DIR, f"{prefix}_separate_wins_counts.csv"))
    print(os.path.join(RESULTS_DIR, f"{prefix}_separate_wins_reason_counts.csv"))
    print(f"\nPresentation-ready plots saved in: {os.path.join(RESULTS_DIR, PRESENTATION_SUBDIR)}")
    for p in presentation_plot_paths:
        if p:
            print(p)
    print(presentation_readme)

    majority_acc = (meta["true_winner"] == "KB").mean()
    best_acc = float(best["accuracy"])
    print(f"\nMajority-KB baseline on decisive JAGT: {majority_acc:.4f}")
    print(f"Best supervised mean holdout accuracy: {best_acc:.4f}")
