...
OUTDIR_ALIGNED = "tsfresh_targeted_clustering_results_aligned"
...
def get_outdir():
    if ANALYSIS_MODE in {"all_timeseries", "steady_jagt_only"}:
        return OUTDIR_ALIGNED
...
def main():
    print("\nAligned hybrid cluster-signature SSD recommendation experiment")
    print("=" * 70)
    print(f"ANALYSIS_MODE = {ANALYSIS_MODE}")
    print(f"OUTDIR        = {OUTDIR}")
    print("Using existing ALIGNED clustering cache; clustering is NOT re-run.")
    ...
    # ------------------------------------------------------------
    # Direct TSFresh diagnostic
    # ------------------------------------------------------------
    print("\n" + "=" * 70)
    print("DIRECT TSFRESH SUPERVISED DIAGNOSTIC")
    print("=" * 70)

    sup_metrics, sup_predictions, sup_feature_importances = run_supervised_holdout(X, meta)

    sup_summary = (
        sup_metrics.groupby(["model", "feature_selection", "top_n"], as_index=False)
        .agg(
            accuracy=("accuracy", "mean"),
            balanced_accuracy=("balanced_accuracy", "mean"),
            macro_f1=("macro_f1", "mean"),
    ...
    sup_summary.to_csv(os.path.join(RESULTS_DIR, f"{prefix}_direct_tsfresh_model_summary.csv"), index=False)
    sup_metrics.to_csv(os.path.join(RESULTS_DIR, f"{prefix}_direct_tsfresh_metrics_all_splits.csv"), index=False)
    sup_predictions.to_csv(os.path.join(RESULTS_DIR, f"{prefix}_direct_tsfresh_predictions_all_test_appearances.csv"), index=False)
    sup_feature_importances.to_csv(os.path.join(RESULTS_DIR, f"{prefix}_direct_tsfresh_feature_importances_all.csv"), index=False)

    print("\nTop direct-TSFresh models:")
    print(sup_summary.head(15).to_string(index=False))

    rf100 = sup_summary[
        (sup_summary["model"] == "random_forest")
        & (sup_summary["feature_selection"] == "mutual_info")
        & (sup_summary["top_n"] == 100)
    ]
    print("\nRandom Forest + MI + top-100 TSFresh:")
    print(rf100.to_string(index=False))
    ...
    summarize_hybrid_feature_importances(feature_importances, best, prefix)
    ...
    print("\nSaved main outputs:")
    print(os.path.join(RESULTS_DIR, f"{prefix}_direct_tsfresh_model_summary.csv"))
