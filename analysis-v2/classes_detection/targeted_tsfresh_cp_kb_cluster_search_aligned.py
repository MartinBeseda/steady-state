...
OUTDIR = "tsfresh_targeted_clustering_results_aligned"
...
def derive_raw_series_keys(glob_pattern):
    """
    Reconstruct the exact per-series key order produced by the current raw-file
    traversal. This is used only to align an already-generated TSFresh cache
    with the canonical key order stored in series_kbkssd.json.

    IMPORTANT: the old pipeline implicitly assumed that glob/file-system order
    matched the JSON key order. That is not portable across machines.
    """
    raw_keys = []
    for fp in glob.glob(glob_pattern):
        ...
        if isinstance(obj, list) and len(obj) > 0 and isinstance(obj[0], list):
            raw_keys.extend(f"{filename}_{i}" for i in range(len(obj)))
        elif isinstance(obj, list):
            raw_keys.append(filename)
        else:
            raise ValueError(
                f"Unsupported JSON structure in {fp}: expected a list or list of lists, "
                f"got {type(obj).__name__}."
            )
    return raw_keys


def align_data_and_feature_cache(raw_data, raw_keys, canonical_keys, features_all):
    """
    Reorder raw time series and cached TSFresh rows to the canonical key order.

    The cached feature rows were produced from raw_data in raw glob order.
    canonical_keys comes from series_kbkssd.json.  We therefore map each
    canonical key back to its raw position instead of assuming positional
    equality between the two sources.
    """
    if len(raw_data) != len(raw_keys):
        raise RuntimeError(
            f"Raw data/key mismatch: {len(raw_data)} time series vs {len(raw_keys)} derived keys."
        )
    if len(features_all) != len(raw_keys):
        raise RuntimeError(
            f"Feature-cache/key mismatch: {len(features_all)} feature rows vs {len(raw_keys)} raw keys."
        )
    if len(canonical_keys) != len(raw_keys):
        raise RuntimeError(
            f"Canonical/raw key-count mismatch: {len(canonical_keys)} vs {len(raw_keys)}."
        )

    if len(set(raw_keys)) != len(raw_keys):
        raise RuntimeError("Duplicate keys found in raw-data key reconstruction.")
    if len(set(canonical_keys)) != len(canonical_keys):
        raise RuntimeError("Duplicate keys found in series_kbkssd.json.")

    raw_pos = {k: i for i, k in enumerate(raw_keys)}
    missing = [k for k in canonical_keys if k not in raw_pos]
    extra = [k for k in raw_keys if k not in set(canonical_keys)]
    if missing or extra:
        raise RuntimeError(
            "Raw/canonical key sets differ. "
            f"Missing canonical keys: {len(missing)} (first: {missing[:5]}). "
            f"Extra raw keys: {len(extra)} (first: {extra[:5]})."
        )

    order = np.asarray([raw_pos[k] for k in canonical_keys], dtype=int)
    positional_matches_before = int(np.sum(np.asarray(raw_keys, dtype=object) == np.asarray(canonical_keys, dtype=object)))

    aligned_data = [raw_data[i] for i in order]
    aligned_features = features_all.iloc[order].reset_index(drop=True).copy()

    print("\nKEY/FEATURE ALIGNMENT CHECK")
    print("-" * 40)
    print(f"Raw series:                  {len(raw_keys)}")
    print(f"Canonical keys:              {len(canonical_keys)}")
    print(f"Positional matches before:   {positional_matches_before}/{len(raw_keys)}")
    print("Alignment by explicit key:   YES")
    print("Feature rows reordered:      YES")
    print("Raw series reordered:        YES")

    return aligned_data, aligned_features
...
def run_targeted_sweep():
    ...
    # Load the raw series in the order returned by the current filesystem/glob.
    # The feature cache was generated from this same traversal order.
    raw_data = cd.load_all_json(DATA_GLOB)
    print(f"Loaded {len(raw_data)} timeseries.")

    canonical_keys = load_series_keys(SERIES_INDEX_PATH, expected_len=len(raw_data))
    raw_keys = derive_raw_series_keys(DATA_GLOB)
    features_all_raw_order = load_cached_features(FEATURE_CACHE_PATH)

    # Critical portability fix: explicitly align both the raw time series and
    # the cached TSFresh rows to the canonical series_kbkssd.json key order.
    full_data, features_all = align_data_and_feature_cache(
        raw_data=raw_data,
        raw_keys=raw_keys,
        canonical_keys=canonical_keys,
        features_all=features_all_raw_order,
    )
    full_keys = canonical_keys
