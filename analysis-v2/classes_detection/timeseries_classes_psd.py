...
if __name__ == '__main__':
    ...
    print('Loading data')
    # no_filenames = 20
    filenames = [fname for fname in os.listdir(f'{data_dir}/timeseries/all')]
    ...
    for filename in filenames:
        for fork in json.load(open(os.path.join(data_dir, 'timeseries/all', filename))):
            ...
            f, psd_vals = welch(fork)

            data.append(psd_vals)
    ...
    with open(f'{dirname}/log.txt', 'w') as f:
        ...
        # Perform HDBSCAN
        print(f'Performing HDBSCAN with {metrics}:')
        hdb = HDBSCAN(metric=similaritymeasures.mae, #lambda x, y: cosine_distances(x.reshape(1, -1), y.reshape(1, -1)), #similaritymeasures.mae,
