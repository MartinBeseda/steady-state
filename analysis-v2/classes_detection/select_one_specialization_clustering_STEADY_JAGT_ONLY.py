...
OUTDIR = "tsfresh_targeted_clustering_results_steady_jagt_only"
...
ONE_DIR = os.path.join(OUTDIR, "ONE_SELECTED_CLUSTERING_STEADY_JAGT_ONLY")
os.makedirs(ONE_DIR, exist_ok=True)

ALPHA = 0.05
MIN_JAGT = 5
TARGET_VARIANT = "steady_jagt_only"
...
def choose_one_clustering(summary_path):
    ...
    # For this post-processing script, intentionally use only the clustering
    # configurations obtained from steady-JAGT-only clustering.
    if "variant" not in df.columns:
        raise ValueError("Expected column 'variant' in the interpretable summary table.")

    df = df[df["variant"] == TARGET_VARIANT].copy()

    if df.empty:
        raise RuntimeError(
            f"No rows found for variant={TARGET_VARIANT!r}. "
            "Run the steady-JAGT-only clustering script first."
        )
...
def validate_steady_jagt_only_selected_result(selected, cached_result, full_cluster_df, jagt_methods):
    """
    Hard safety checks for the steady-JAGT-only post-processing pipeline.

    This script must post-process a clustering that was computed only on steady
    JAGT series. Therefore:
      1) selected.variant must be steady_jagt_only;
      2) cached_result.variant must be steady_jagt_only;
      3) every cached series key must exist in the JAGT method table;
      4) the number of cached labels must equal the number of cached keys;
      5) the cluster table must not contain non-JAGT clustered points.

    If any of these checks fails, stop immediately instead of silently producing
    misleading plots from an all-timeseries clustering.
    """
    selected_variant = selected["variant"] if isinstance(selected, dict) else selected["variant"]

    if selected_variant != TARGET_VARIANT:
        raise RuntimeError(
            f"Selected variant is {selected_variant!r}, expected {TARGET_VARIANT!r}. "
            "This is not a steady-JAGT-only selection."
        )

    if getattr(cached_result, "variant", None) != TARGET_VARIANT:
        raise RuntimeError(
            f"Cached result variant is {getattr(cached_result, 'variant', None)!r}, "
            f"expected {TARGET_VARIANT!r}. Delete the wrong cache or rerun the strict steady-JAGT clustering script."
        )

    if len(cached_result.labels) != len(cached_result.series_keys):
        raise RuntimeError(
            f"Cached label/key mismatch: labels={len(cached_result.labels)}, "
            f"keys={len(cached_result.series_keys)}."
        )

    if len(cached_result.data_normalized) != len(cached_result.series_keys):
        raise RuntimeError(
            f"Cached data/key mismatch: data={len(cached_result.data_normalized)}, "
            f"keys={len(cached_result.series_keys)}."
        )

    missing = [k for k in cached_result.series_keys if k not in jagt_methods.index]
    if missing:
        preview = missing[:10]
        raise RuntimeError(
            f"Cached clustering contains {len(missing)} non-JAGT series keys. "
            f"First examples: {preview}. This means it is not truly steady-JAGT-only."
        )

    labels = np.asarray(cached_result.labels)
    label_counts = pd.Series(labels).value_counts().to_dict()

    table_counts = dict(zip(full_cluster_df["cluster"], full_cluster_df["cluster_size"]))
    for cid, count in label_counts.items():
        if cid not in table_counts:
            raise RuntimeError(
                f"Cluster {cid} exists in cached labels but not in the cluster table."
            )
        if int(table_counts[cid]) != int(count):
            raise RuntimeError(
                f"Cluster {cid} size mismatch: table={table_counts[cid]}, cached labels={count}."
            )

    # In a true steady-JAGT-only clustering, every clustered point is a JAGT series.
    # Therefore n_jagt must equal cluster_size for every cluster, including outliers.
    bad_rows = full_cluster_df[full_cluster_df["n_jagt"] != full_cluster_df["cluster_size"]]
    if not bad_rows.empty:
        raise RuntimeError(
            "Cluster table contains non-JAGT points: expected n_jagt == cluster_size for every cluster.\n"
            + bad_rows[["cluster", "cluster_size", "n_jagt"]].to_string(index=False)
        )

    print("\nSTRICT STEADY-JAGT-ONLY CHECKS PASSED")
    print(f"Selected variant: {selected_variant}")
    print(f"Cached result variant: {cached_result.variant}")
    print(f"Clustered series: {len(cached_result.series_keys)}")
    print("All cached series keys are present in the JAGT method table: YES")
    print("For every cluster, n_jagt == cluster_size: YES")
...
def plot_all_cluster_sizes(all_clusters, prefix):
    ...
    ax.bar(x - width / 2, df["cluster_size"], width, label="cluster size")
    ax.bar(x + width / 2, df["n_jagt"], width, label="JAGT series")
    ...
    ax.set_xlabel("Cluster")
    ax.set_ylabel("Count")
    ax.set_title("Selected steady-JAGT-only clustering: cluster sizes and JAGT support")
...
def make_text_summary(selected, all_clusters, kb_row, cp_row, copied, common_outputs):
    ...
    lines.append("- Clustering was performed only on steady JAGT series.")
...
def main():
    ...
    validate_steady_jagt_only_selected_result(
        selected=selected,
        cached_result=cached_result,
        full_cluster_df=full_cluster_df,
