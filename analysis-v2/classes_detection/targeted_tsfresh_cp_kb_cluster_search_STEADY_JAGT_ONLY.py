...
This patched version evaluates only:
    steady_jagt_only
...
def build_steady_jagt_variant(full_data, full_keys, features_all, jagt_methods):
    """
    Build exactly one dataset: steady JAGT series only.

    IMPORTANT:
    - PCA/UMAP/HDBSCAN are fitted only on this filtered subset.
    - No all-timeseries clustering is performed anywhere in this script.
    - Every clustered point must have a key present in jagt_methods.
    """
    ...
    if len(features_all) != len(keys_drop):
        raise ValueError(
            f"Feature-cache length mismatch: features={len(features_all)}, "
            f"keys after drop={len(keys_drop)}. Expected cached drop_first1 features."
        )

    jagt_mask = np.array([k in jagt_methods.index for k in keys_drop], dtype=bool)
    jagt_indices = np.where(jagt_mask)[0]

    if len(jagt_indices) == 0:
        raise RuntimeError("No steady JAGT series found in the cached feature/key alignment.")

    features_jagt = features_all.iloc[jagt_indices].reset_index(drop=True).copy()
    keys_jagt = [keys_drop[i] for i in jagt_indices]
    data_jagt = [data_norm[i] for i in jagt_indices]

    # Hard safety checks.
    if len(features_jagt) != len(keys_jagt) or len(keys_jagt) != len(data_jagt):
        raise RuntimeError(
            "Internal alignment error after JAGT filtering: "
            f"features={len(features_jagt)}, keys={len(keys_jagt)}, data={len(data_jagt)}"
        )

    missing = [k for k in keys_jagt if k not in jagt_methods.index]
    if missing:
        raise RuntimeError(
            "The steady-JAGT-only dataset still contains non-JAGT keys. "
            f"First offending keys: {missing[:5]}"
        )

    print("\nSTEADY-JAGT FILTERING CHECK")
    print("-" * 40)
    print(f"Full drop-first dataset size: {len(keys_drop)}")
    print(f"Steady JAGT subset size:     {len(keys_jagt)}")
    print(f"Feature matrix used:         {features_jagt.shape}")
    print("All clustered series are steady JAGT: YES")

    return {
        "variant": "steady_jagt_only",
        "features": features_jagt,
        "series_keys": keys_jagt,
        "data_normalized": data_jagt,
    }
...
def run_targeted_sweep():
    ...
    # Build exactly one clustering input: steady JAGT only.
    var = build_steady_jagt_variant(full_data, full_keys, features_all, jagt_methods)
    ...
    # Final hard check before any PCA/UMAP/HDBSCAN call.
    assert variant == "steady_jagt_only"
    assert len(features) == len(series_keys) == len(data_normalized)
    assert all(k in jagt_methods.index for k in series_keys)
    ...
    print("THIS IS THE ONLY VARIANT BEING CLUSTERED.")
    print("=" * 90)
    ...
    # Safety: this output must not contain all_timeseries.
    bad_variants = sorted(set(df_metrics["variant"]) - {"steady_jagt_only"})
    if bad_variants:
        raise RuntimeError(f"Unexpected non-JAGT variants found in metrics: {bad_variants}")
    ...
    for result, cluster_eval in all_results:
        if result.variant != "steady_jagt_only":
            raise RuntimeError(f"Unexpected cached result variant: {result.variant}")
        if len(result.series_keys) != len(features):
            raise RuntimeError(
                "Cached result size mismatch. Delete the configuration_cache folder and rerun. "
                f"cached={len(result.series_keys)}, expected={len(features)}"
            )
...
def write_readme(df_metrics):
    ...
    lines = [
        ...
        "- Evaluate only steady-JAGT-only clustering.",
        "",
        "Primary score:",
        "- hypothesis_score = 0.45 * internal_score + 0.55 * method_signal",
